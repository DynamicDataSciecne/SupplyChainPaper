...
library(fGarch) # for studnet t distribution
library(forecast)
library(astsa)
...
# n=500
set.seed(2021)
D.t <- AR_sim(t=1000, mean = 60, ar.coef = 0.80, order = c(1, 0, 0), error = "t", df=3) # AR(1)
#hist(D.t)
#head(D.t)
...
################################################################################
# New simulation for journal paper
# date: June 11, 2021
# Seasonal and nonstationary
################################################################################

################################################################################
# M3:  # Generate data from ARIMA model (nonstationary) 
################################################################################
#random walk model (d_t = d_{t−1} + a_t), a_t∼N(0,q).
# n=500
#set.seed(2021)
#Dt <- at <- rnorm(n = 500, mean = 0, sd = 1)
#D.t <- cumsum(at) ## simulate RW
#set.seed(2021)
#D.t <- arima.sim(model= list(order = c(0, 1, 0)), n=400, mean=0, sd =  8)
#head(D.t)
#arima(D.t, order = c(0,1,0))

# # simple non-stationary time series model: An ARIMA simulation
# n=1000
set.seed(2021)
D.t <- arima.sim(list(order = c(1,1,0), ar = 0.8), n = 1000, mean=0, sd =  20)
#plot.ts(D.t)
...
# n=2000
set.seed(2021)
D.t <- arima.sim(list(order = c(1,1,0), ar = 0.8), mean=0, n = 2000)

# n=3000
set.seed(2021)
D.t <- arima.sim(list(order = c(1,1,0), ar = 0.8), mean=0, n = 3000)
...
D.t <- diff(D.t) #generate the first difference of the data
...
################################################################################
# M4:  # Generate data from seasonal ARIMA model
################################################################################

# Example: ARIMA(0, 0, 1) x (0, 0, 1)12
#The model includes seasonal and non-seasonal MA(1) terms, no differencing and no AR terms
D.t <- arima.sim(list(order = c(0,0,12), ma = c(0.7,rep(0,10),0.9)), n = 200)
acf2(D.t)

#Example: ARIMA(1, 0, 0) x (1, 0, 0)12
#A seasonal AR(1) model has significant lags at 1, 12 and 13, with an increasing trend 
# before lag 12, sharply cut off at lag 13.
set.seed(2021)
D.t <- arima.sim(list(order = c(12,0,0), ar = c(.5,rep(0,10),.5)), n = 2000, n.start = 12)

plot.ts(D.t[1:100], ylab = paste("AR(", order[1], ")", sep = ""))
acf2(D.t)

#acf(arima.sim(model = list(ar = c(0.5,rep(0,10),0.5,-0.5*0.5)), n = 100, n.start = 13))

D.t <- diff(D.t,12) #generate the first difference of the data
